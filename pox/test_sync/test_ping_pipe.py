...
def test_edge_switch (event):
	...
	msg = cfg.add_classfier_table('classfier_edge6')
	event.connection.send(msg)

	msg = cfg.add_classfier_entry('classfier_edge6', 0)
	event.connection.send(msg)

	msg = cfg.add_classfier_entry('classfier_edge6', 1)
	event.connection.send(msg)

	msg = tab_cfg.add_flow_table('pipe_edge3')
	event.connection.send(msg)

	msg = tab_cfg.add_flow_entry('pipe_edge3', 0)
	event.connection.send(msg)
	msg = tab_cfg.add_flow_entry('pipe_edge3', 1)
	event.connection.send(msg)
