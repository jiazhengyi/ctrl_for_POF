...
def add_time_slot (slot): 
	...
	tslot_origin = g.tslot_unused;
	for i in slot.keys():
		tag_info = of.sync_tag_info()
		tag_info.tag = slot[i][0]
		tag_info.port = slot[i][1]
		
		if (slot[i][2] > len(g.tslot_unused)):
			print "the time slot not enough!"

		tag_info.tslot = g.tslot_unused[:slot[i][2]]
		msg.tslot_cfg.data.append(tag_info)
		# updata the time slot used information
		g.tslot_unused = g.tslot_unused[slot[i][2]:]

	msg.tslot_cfg.tag_num = len(msg.tslot_cfg.data) 
	
	g.tslot_unused = tslot_origin;
