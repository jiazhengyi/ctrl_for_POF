...
def test_core0_switch (event):
	...
	msg = cfg.add_time_slot(g.tslot_core0)
	event.connection.send(msg)

	msg = tab_cfg.add_flow_table('pipe_core0')
	event.connection.send(msg)

	msg = tab_cfg.add_flow_entry('pipe_core0', 0)
	event.connection.send(msg)
	msg = tab_cfg.add_flow_entry('pipe_core0', 1)
	event.connection.send(msg)

	msg = cfg.add_classfier_table('classfier_core0')
	event.connection.send(msg)

	msg = cfg.add_classfier_entry('classfier_core0', 0)
	event.connection.send(msg)
