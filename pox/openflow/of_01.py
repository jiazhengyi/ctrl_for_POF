...
unpackers = make_type_to_unpacker_table() #define in openflow.util
...
def handle_GET_CONFIG_REPLY(con,msg):
    """
    @author: milktank
    @contact: add get_config_request just to compare the switch CONFIG if we configure the switch 
    right
    @version:1.0
    @edit_time:2014-10-1 14:48:55
    """
    con.config = msg

def handle_GET_CONFIG_REPLY (con, msg):   # cc
    print "receive Get_config_Reply message"   #cc
...
def handle_FEATURES_REPLY (con, msg):
	...
	#con.ports._reset()
	con.dpid = msg.deviceId
	e = con.ofnexus.raiseEventNoErrors(SyncTransUp, con, msg)
	if e is None or e.halt != True:
		con.raiseEventNoErrors(SyncTransUp, con, msg)
...
def handle_FEATURES_REPLY (con, msg):
  ...
  #con.ports._reset()
  con.dpid = msg.deviceId
  con.has_receive_port_status_num=0
  if not connecting:
  ...
  if con.ofnexus.clear_flows_on_connect:
    con.send(of.ofp_get_config_request())
...
def handle_PORT_STATUS (con, msg): #A
  ...
  con.has_receive_port_status_num +=1
  if con.has_receive_port_status_num == con.features.portNum:   ##add to receive port_status
    #add_first_table(con)
    #send_port_mod(con)
    e = con.ofnexus.raiseEventNoErrors(ConnectionUp, con, msg)  ##add to raise ConnectionUp event
    if e is None or e.halt != True:                             ##try the raise the connectionUp event                 
        con.raiseEventNoErrors(ConnectionUp, con, msg)
    con.connect_time = time.time()
...
#new handler for ofp_queryall_request

def handle_table_mod(con, msg):
    print ""
    print "=========================================================="
    print "Receive a table message:"
    print msg.show()
    print "=========================================================="

def handle_flow_mod(con, msg):
    print ""
    print "=========================================================="
    print "Receive a flow message:"
    print msg
    print "=========================================================="
...
def handle_counter_reply(con, msg):
    print ""
    print "=========================================================="
    print "Receive a counter_reply message:"
    print msg
    print "=========================================================="



def handle_query_fin(con, msg):
    print ""
    print "=========================================================="
    print "Receive a query_fin message:"
    print msg
    print "=========================================================="

def handle_exper_msg (con, msg):
    print ("handle expermenter message!\n")
...
handlerMap = {
  ...
  #of.OFPT_VENDOR : handle_VENDOR, # modify by jiazy

#  @author: milktank
#  @contact: add new handle_message func 
#  @version:1.1
#  @edit_time:2014-10-1 14:46:48
  of.OFPT_GET_CONFIG_REPLY : handle_GET_CONFIG_REPLY,
  of.OFPT_RESOURCE_REPORT : bpm.handle_RESOURCE_REPORT,  #cc
  
#  @author: milktank
#  @contact: add new handle_message func for query
#  @version:1.2
#  @edit_time:2014-12-3
  of.OFPT_TABLE_MOD : handle_table_mod  ,
  of.OFPT_FLOW_MOD : handle_flow_mod  ,
  of.OFPT_METER_MOD : handle_meter_mod  ,
  of.OFPT_GROUP_MOD : handle_group_mod  ,
  of.OFPT_COUNTER_REPLY:bpm.handle_COUNTER_REPLY,   #cc
  of.OFPT_QUERY_FIN:handle_query_fin,

#  @author: jiazy
#  @contact: add new OFPT_EXPER func for sync transmission
#  @edit_time:2016-12-6

  of.OFPT_EXPER : handle_exper_msg ,
...
class PortCollection (object):
  ...
  def _forget (self, portId):
    self._masks.add(portId)
    self._ports = set([p for p in self._ports if p.portId != portId])

  def _update (self, port):
    self._masks.discard(port.portId)
    self._ports = set([p for p in self._ports if p.portId != port.portId])
  ...
  def __getitem__ (self, index):
    ...
    if self._chain:
      p = self._chain[index]
      if p.portId not in self._masks:
        return p
  ...
  def keys (self):
    if self._chain:
      ...
      k = set()
    k.update([p.portId for p in self._ports])
    return list(k)
...
class Connection (EventMixin):
  ...
  def read (self):
    ...
    while buf_len - offset >= 8: # 8 bytes is minimum OF message size
      ...
      msg_length = ord(self.buf[offset+2]) << 8 | ord(self.buf[offset+3]) #network bytes to host bytes
...
class OpenFlow_01_Task (Task):
  ...
  """
  def __init__ (self, port = 6636, address = '0.0.0.0'):
    Task.__init__(self)
    self.port = int(port)
...
def launch (port = 6636, address = "0.0.0.0"):
