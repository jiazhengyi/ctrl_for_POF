...
class LLDPSender (object):
  ...
  def _handle_openflow_PortStatus (self, event):
    ...
    #print ('PortStatus')
    if event.added:
      self.add_port(event.dpid, event.port, event.ofp.desc.hardwareAddress)
    elif event.deleted:
  ...
  def _handle_openflow_ConnectionUp (self, event):
    ...
    ports = [(p.desc.portId, p.desc.hardwareAddress) for p in event.connection.phyports]
  ...
  def _timer_handler (self):
    ...
    the next cycle.
    """
    #print ('_this_cycle:', self._this_cycle)
    #print ('_next_cycle:', self._next_cycle)
  ...
  def create_discovery_packet (self, dpid, port_num, port_addr):
    ...
    po = of.ofp_packet_out(action = of.ofp_action_output(portId=port_num))
    #po = of.ofp_packet_out()
    #po.actions.append(of.ofp_action_output(portId=port_num))
    po.data = eth.pack()
    return po.pack()
...
class Discovery (EventMixin):
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    """
    #print ("Receive and process LLDP packets")
    ...
    if self._explicit_drop:
      if event.ofp.bufferId is not None:
        #print ("---------------------")
        log.debug("Dropping LLDP packet %i", event.ofp.bufferId)
        '''
