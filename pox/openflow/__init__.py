...
class ConnectionReady(Event):    # add by lsr
    """
...
class PortStatus (Event):
  ...
  def __init__ (self, connection, ofp):
    ...
    self.port = ofp.desc.portId    #modify by Tan
...
class PacketIn (Event):
  ...
  def __init__ (self, connection, ofp):
    ...
    self.connection = connection
    self.ofp = ofp
    self.port = ofp.port_id
    self.data = ofp.packetData
    self._parsed = None
    self.dpid = connection.dpid
